...
function Page() {
  ...
  const [privateReservation, setPrivateReservation] = useState([]);
  const [fetching, setFetching] = useState(true);

  useEffect(() => {
    async function fetchPrivateReservation() {
      setFetching(true);
      try {
        const res = await fetch("/api/privatereservation");
        if (!res.ok) throw new Error("Failed to fetch private reservation");
        const data = await res.json();
        setPrivateReservation(data.privateReservations);
        console.log(data.privateReservations);
      } catch (error) {
        console.error("Error fetching private reservation:", error);
      } finally {
        setFetching(false);
      }
    }
      ...
      {fetching ? (
        ...
        <div className="overflow-auto">
          <table className="w-full border-collapse border border-gray-500">
            ...
            <tbody>
              {privateReservation.slice().reverse().map((reservation, index) => (
